...
import numpy as np
import os
from need_module_corrected import image_transform,infer, load_engine, prepare_buffers, ImageFolderWithPath
import torch
import time
...
teacher_engine = load_engine('teacher.trt')
student_engine = load_engine('student.trt')
autoencoder_engine = load_engine('auto.trt')


teacher_inputs, teacher_outputs, teacher_bindings, teacher_stream = prepare_buffers(teacher_engine)
student_inputs, student_outputs, student_bindings, student_stream = prepare_buffers(student_engine)
autoencoder_inputs, autoencoder_outputs, autoencoder_bindings, autoencoder_stream = prepare_buffers(autoencoder_engine)

def predict_trt(image, teacher_context, student_context, autoencoder_context,
                teacher_mean, teacher_std, q_st_start=None, q_st_end=None, q_ae_start=None, q_ae_end=None):
    # 입력 이미지를 호스트 메모리로 복사
    out_channels = 384
    np.copyto(teacher_inputs[0].host, image.ravel())
    np.copyto(student_inputs[0]['host'], image.ravel())
    np.copyto(autoencoder_inputs[0]['host'], image.ravel())
    
    # 추론 실행
    teacher_output = infer(teacher_context, teacher_bindings, teacher_inputs, teacher_outputs, teacher_stream)
    student_output = infer(student_context, student_bindings, student_inputs, student_outputs, student_stream)
    autoencoder_output = infer(autoencoder_context, autoencoder_bindings, autoencoder_inputs, autoencoder_outputs, autoencoder_stream)
    teacher_output_np = teacher_output[0]  # Get the numpy array from the list
    teacher_output_tensor = torch.from_numpy(teacher_output_np).to('cuda:0')  # Convert to tensor and move to GPU
    teacher_output_tensor = (teacher_output_tensor - teacher_mean) / teacher_std  # Perform the operation
    student_output_tensor = student_output[0]  # Get the tensor from the list
    map_st = torch.mean((teacher_output - student_output_tensor[:, :out_channels])**2, dim=1, keepdim=True)
    map_ae = torch.mean((autoencoder_output -
                         student_output_tensor[:, out_channels:])**2,
                        dim=1, keepdim=True)
...
def visualize_one_sample(test_data, orig_height, orig_width, autoencoder_context, student_context, teacher_context):
    with torch.no_grad():
        ...
        map_combined, map_st, map_ae = predict_trt(test_data, teacher_context=teacher_context, student_context=student_context, autoencoder_context=autoencoder_context, teacher_mean=tea_mean, teacher_std=tea_std)
...
if __name__ == '__main__':
    ...
    with teacher_engine.create_execution_context() as teacher_context, \
     student_engine.create_execution_context() as student_context, \
     autoencoder_engine.create_execution_context() as autoencoder_context:
        for image, _,_ in test_set:
            orig_width = image.width
            orig_height = image.height
            image = image_transform(image)

            y_class, fps, map_combined = visualize_one_sample(image,orig_height=orig_height, orig_width=orig_width, autoencoder_context=autoencoder_context
                                                              ,student_context=student_context, teacher_context=teacher_context)
            print(fps)
