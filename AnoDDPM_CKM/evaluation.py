...
# recon: 모델이 생성한 불량 부분 mask 이미지
# real: 입력 이미지(실제 이미지)
# real_mask: 입력 이미지 (실제 이미지)의 불량 부분 mask 이미지
# 만약 우리가 원하는 결과값이 나오는 모델이라면 일단 저걸로 0,1이 분류 가능해야하고
# 그걸 통해서 평가 산식을 만들어야하는데 저게 지금 답이 없으니 아예 평가하는 걸 바꿔야한다
# 그럴려면 모델 output을 먼저 알고 그다음에 그 output을 어떻게 0,1로 바꿀건지
# 그다음에 평가산식 건들기 해야해
...
def AUC_score_100(fpr, tpr):
    return 100*auc(fpr, tpr)
