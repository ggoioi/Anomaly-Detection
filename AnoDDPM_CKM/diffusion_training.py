...
# 학습을 담당하는 주요 함수 주어진 데이터 셋 로더를 사용해 모델을 학습하고 테스트
# args는 학습 및 모델 설정을 담은 딕셔너리 이며 resume은 이전 학습을 재개 하는 데 필요한 파라미터
def train(training_dataset_loader, testing_dataset_loader, args, resume):
    ...
    # UNet 모델 초기화 - 사전에 지정되어 있는 args와 UNetModel class 활용해서 모델을 생성한다.
    model = UNetModel(
    ...
    # diffusion 모델에서 사용되는 beta 값의 일련의 스케줄을 얻기 위한 부분으로 추정 됨
    # diffusion 모델은 diffusion process을 모델링 하는데 사용되며, 이런 확산(diffusion) 과정을 통해 데이터를 변환하고 노이즈를 주입한다!
    betas = get_beta_schedule(args['T'], args['beta_schedule'])


    # diffusion 모델 지정 - 사전에 설정해둔 beta 값과 args의 값을 통해 확산 모델을 초기화한다.
    diffusion = GaussianDiffusionModel(
            args['img_size'], betas, loss_weight=args['loss_weight'],
            loss_type=args['loss-type'], noise=args["noise_fn"], img_channels=in_channels
            )

    # resume이 true인 경우 = 이전 학습을 재개하는 경우? resume에 값이 있는 경우
    # 사전에 저장된 모델 가중치가 있는 경우 ema를 사전 저장된 모델로 지정
    if resume:
    ...
    # 아니면 epoch 0으로 초기화 하고 ema는 우리가 위에서 설정했던 UNetModel로 초기화
    else:
        start_epoch = 0
        ema = copy.deepcopy(model)

    # train epoch, optimizer 설정 resume값이 있는 경우 해당 값으로 optimizer 설정
    ...
    start_time = time.time()  # 경과 시간 측정을 위한 시작 time 기억
    losses = []
    vlb = collections.deque([], maxlen=10)

    # 데이터셋과 args의 배치 크기에 맞는 iterator 지정
...
def main():
    ...
    if args["dataset"].lower() == "cifar":
        ...
        training_dataset_loader = dataset.init_dataset_loader(training_dataset, args)
        testing_dataset_loader = dataset.init_dataset_loader(testing_dataset, args)
    elif args["dataset"].lower() == "ckm":  # 환편기 데이터 training, test set
        training_dataset = dataset.CKMDataset(
                "./ckm_dataset", 
                False, 
                args["img_size"],
                False
        ...
        testing_dataset = dataset.CKMDataset(
                "./ckm_dataset",
                True,
                args["img_size"],
                False,
        ...
        print("test:", len(testing_dataset), end='\n\n')
    else:
        # load NFBS dataset
